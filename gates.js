...
class interfaz{
  ...
  generarPNG(){
      const clipX = 0;
      const clipY = 0;
      const clipWidth = 2*this.X + this.esatadoActual.matriz[0].length*this.w;
      const clipHeight = 2*this.Y-2*this.espacio+ this.esatadoActual.matriz.length*(this.h+this.espacio);
      ...
      tempContext.fillStyle = "#f0f3f3"; 
      tempContext.fillRect (0,0,tempCanvas.width,tempCanvas.height) 
      tempContext.drawImage(this.canvas, 0, 0);

      // Obtener la representación en base64 del canvas recortado como PNG
      const clippedImageData = tempCanvas.toDataURL('image/png');

      // Crear un enlace de descarga
      const link = document.createElement('a');
      link.href = clippedImageData;
      link.download = 'clipped.png';
  ...
  simularMultiple(){
    ...
    reader.onload = () => {
      ...
      this.log(`El proceso tardó ${tiempoTranscurrido} milisegundos.`);
    }
    reader.readAsText(file);
...
//        let num1 = 1-num2;
  ...
  actualizarDiagrama(){    
    ...
    /////////////////////////DIBUJO
    this.ctx.fillStyle = "#f0f3f3"; 
    //this.ctx.fillRect (0,0,this.Wcanvas,this.Hcanvas) clearRect
    console.log("TAMAGNO DEL FILLRECT:",this.Wcanvas,this.Hcanvas)
